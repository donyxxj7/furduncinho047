    ...
    register: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const existingUser = await dbInstance
          .select()
          .from(users)
          .where(or(eq(users.email, input.email), eq(users.cpf, input.cpf)))
          .limit(1);

        if (existingUser.length > 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "E-mail ou CPF já cadastrados no sistema.",
          });
        }
    ...
    validate: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!ticket || ticket.status !== "paid") {
          return { valid: false, message: "QR Code inválido ou já utilizado" };
        }
        ...
        await db.createCheckinLog({
          ...
          adminId: ctx.user.id,
          result: "valid",
          notes: ticket.hasCooler ? "INGRESSO COM COOLER" : "INGRESSO NORMAL",
        });
